...
st.title("🤖 LLM Application Security")

model_choice = st.sidebar.selectbox(
    "Choose a model",
    ["Claude 3.5 Sonnet", "Claude v2", "DeepSeek-V2 Chat"],
    key="model_selector_llm"
)
...
prompt = st.text_area("Enter your LLM application security-related question")
if st.button("Submit"):
        ...
        with st.spinner("Contacting model..."):
            try:
                ...
                if response.status_code == 200:
                    ...
                    if st.button("🔊 Read aloud", key="read_response_llm"):
                        speak_text(output)
                else:
